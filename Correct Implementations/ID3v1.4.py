...
Version 1.2:-
                ...
                2)Now target column can be at any location.
...
def DecisionTreeLearn(attributes,node,stopper,targetIndex):
    if targetEntropy(node.data,targetIndex)==0 or node.depth>=stopper:
        if Count(node.data,targetIndex,targetLabels[0])>Count(node.data,targetIndex,targetLabels[1]):
            node.name=targetLabels[0]
        elif Count(node.data,targetIndex,targetLabels[0])==Count(node.data,targetIndex,targetLabels[1]):
            node.name="Can't Decide! "
        else:
            node.name=targetLabels[1]
