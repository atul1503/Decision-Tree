...
def targetCount(subset,targetIndex):  #gets the count of positive and negative of target attribute
    ...
    for i in range(1,len(subset)):
        if subset[i][targetIndex]=='yes':
            pos=pos+1
        elif subset[i][targetIndex]=='no':
            neg=neg+1
    return pos,neg
...
def attValueCount(subset,index,value,targetIndex):
    ...
    neg=0
    for i in range(len(subset)):
        if subset[i][index]==value and subset[i][targetIndex]=='yes':
            pos=pos+1
        elif subset[i][index]==value and subset[i][targetIndex]=='no':
            neg=neg+1
    return pos,neg
...
def DecisionTreeLearn(attributes,node,stopper,targetIndex):
    if targetEntropy(node.data,targetIndex)==0 or node.depth>=stopper:
        if Count(node.data,targetIndex,'yes')>Count(node.data,targetIndex,'no'):
            node.name='yes'
        elif Count(node.data,targetIndex,'yes')==Count(node.data,targetIndex,'no'):
